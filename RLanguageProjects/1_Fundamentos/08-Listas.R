...
# Listas Compostas - tipos de dados diferentes
lista_composta1 = list("A", 3, TRUE) #string, numero, booleano
lista_composta1

#essa lista tem um vetor de numero, vetor de strings e uma dist Normal
...
lista1 #nomeou lista que ja existia

vec_num <- 1:4
vec_char <- c("A", "B", "C", "D")

lista2 <- list(Numeros = vec_num, Letras = vec_char)
lista2 #nomeou a lista no momento da criação dessa
...
mat = matrix(1:4, nrow = 2) #cria matriz
mat
vec = c(1:9) #cria vetor
vec
lst = list(mat, vec) #une os 2
lst
