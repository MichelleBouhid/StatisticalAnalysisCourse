...
pesq = cbind(pesquisa, olhos) #ligação entre 2 objetos numa coluna
pesq
...
tail(pesq)
tail(mtcars) #dataset vem com o R


# Data frames built-in do R
?mtcars
mtcars
View(mtcars) #abre em formato de tabela

# Filtro para um subset de dados que atendem a um critério
pesq[altura < 1.60,] #usando entre colchetes como no indice usa uma regra
...
colnames(pesq) <- c("Var 1", "Var 2", "Var 3", "Var 4", "Var 5") #muda nome coluna
...
View(pacientes) #atençao a funçao View é com letra maiuscula
head(pacientes)
summary(pacientes)
