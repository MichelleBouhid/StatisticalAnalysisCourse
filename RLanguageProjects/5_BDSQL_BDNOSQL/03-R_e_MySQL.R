...
install.packages("dbplyr") #pacote com o drive para conectar no BDrelacional
library(RMySQL)
library(ggplot2)
library(dbplyr)
library(dplyr)
...
# como o SqLight foi instalado com criptografia precisa trocar no painel de comando
?dbConnect
con = dbConnect(MySQL(), user = "root", password = "dsa1234", dbname = "titanicDB", host = "localhost")
#host- maquina onde esta instalado o BD, não precisa ser o mesmo compt
...
dados2 <- con2 %>%  #simbolo de concatenação
  tbl("titanic") %>%
