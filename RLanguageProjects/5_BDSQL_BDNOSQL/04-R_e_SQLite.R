...
# Executando consultas no banco de dados !Cuidado se o BD for mto grande
query = "select row_names from mtcars"
rs = dbSendQuery(con, query)
dados = fetch(rs, n = -1) #rs=result set
dados
class(dados)

# Executando consultas no banco de dados ! Pode fazer linha e linha
query = "select row_names from mtcars"
rs = dbSendQuery(con, query)
...
# Criando driver e conexão ao banco de dados
drv = dbDriver("SQLite")
con = dbConnect(drv, dbname = "exemplo.db") #o banco ja existe, criamos acima

dbWriteTable(con,"indices", "indice.csv",
              sep = "|", header = T)

dbRemoveTable(con, "indices") #se fez errado pode remover
