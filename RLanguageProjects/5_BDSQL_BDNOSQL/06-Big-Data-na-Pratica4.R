...
setwd("C:/DSAdriveC/LiguagemR/Cap06")
getwd()
...
library(gridExtra)
library(ggthemes)
library(caret) #pacote de ML para R
library(MASS)
...
# "streamingMovies".
str(churn)
cols_recode1 <- c(10:15) #fazemos um vetor
...
# podemos agrupá-los em cinco grupos de posse (tenure): (tempo do contrato)
# “0-12 Mês”, “12–24 Mês”, “24–48 Meses”, “48–60 Mês” Mês ”,“> 60 Mês”

# Exibindo o valor mínimo e máximo de 'tenure'
...
# Aqui, a função sapply aplica a função group_tenure a cada valor da variável tenure no dataset churn.
# O resultado é armazenado em uma nova coluna chamada tenure_group, onde cada cliente é classificado 
# de acordo com seu tempo de permanência em uma das faixas definidas.
churn$tenure_group <- sapply(churn$tenure,group_tenure)

# Convertendo a nova coluna 'tenure_group' para o tipo fator
# Isso facilita o trabalho com categorias em análises e visualizações
...
# Nessa fase buscmaos as relações entre as variáveis, i.é. colunas.
...
# Então, um deles será removido do modelo. Nós removemos Total Charges, evitando multicolinearidade
...
intrain <- createDataPartition(churn$Churn,p=0.7,list=FALSE) #70% para treino, do pacote caret
set.seed(2017) #para reproduzir o mesmo resultado que o professor na aula
training <- churn[intrain,] #intrain foi 70% 
testing <- churn[-intrain,] # - intrain os 30%
...
#colocando no padrao 0 ou 1 para sim ou não
training$Churn <- ifelse(training$Churn == "Yes", 1, 0)
class(training$Churn)
...
# algoritmo é uma formula matematica, que alimentamos com dados e durante o treinamento 
# ele vai aprender os melhores coeficientes que ajudam a completar essa fórmula matematica,
# que preenchem essa formula. O modelo é essa formula matematica preenchida.
# depois que encontrou a formula, limentamos o modelo com novos dados para fazer previsões
...
?anova
anova(LogModel, test="Chisq") #teste Qui quadrado


# Analisando a tabela de variância, podemos ver a queda no desvio (variancia) ao adicionar cada variável 
